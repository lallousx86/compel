...
*/
const CharPair * my_getextendedkeycode(KEY_EVENT_RECORD *);



static int my_getch_nl(void)
{
  ...
  HANDLE coninpfh;
  //        coninpfh = GetStdHandle(STD_INPUT_HANDLE);
  coninpfh = CreateFileA( "CONIN$",
  ...
  */
  GetConsoleMode(coninpfh, &oldstate);
  SetConsoleMode(coninpfh, 0L);
    ...
    */
    if ( (ConInpRec.EventType == KEY_EVENT) &&
      ConInpRec.Event.KeyEvent.bKeyDown ) {
        ...
        */
        if ( pCP = my_getextendedkeycode( &(ConInpRec.Event.KeyEvent) ) ) {
          ch = pCP->LeadChar;
  ...
  */
  SetConsoleMode( coninpfh, oldstate );

  ::CloseHandle(coninpfh);
...
int my_getch (void)
{
  ...
  ch = my_getch_nl();               /* input the character */
...
const CharPair * my_getextendedkeycode (
