...
uses uCanvasExt, CompelScript, compel_lib, Classes, SysUtils, Windows, Graphics, Forms;
...
type
  TCompelCanvasExt = class
    private
      ...
      function FindCanvasById(Idx: Integer): TCanvasExt;
      function FindCanvas(ACanvas: TCanvasExt): Integer;
    public
...
function TCompelCanvasExt.FindCanvas(ACanvas: TCanvasExt): Integer;
var
  i: Integer;
begin
...
function TCompelCanvasExt.FindCanvasById(Idx: Integer): TCanvasExt;
begin
  Result := nil;
...
begin
  ...
  L.Add(TClsFnc.Create('Rectangle', ext_DrawRect, 4, 5));
...
begin
  ...
  Len := Length(Args);
  if Len >= 1 then
    Init.Width := TCompelScript.ParseNumber(Args[0]);

  if Len >= 2 then
    Init.Height := TCompelScript.ParseNumber(Args[1]);

  if Len >= 3 then
    Init.X := TCompelScript.ParseNumber(Args[2]);

  if Len >= 4 then
    Init.Y := TCompelScript.ParseNumber(Args[3]);

  // Create new canvas
  Canvas := TCanvasExt.Create(Init);
...
function TCompelCanvasExt.ext_DrawRect(Script: TCompelScript;
  Args: array of string): Integer;
// Syntax:
// Canvas.Rectangle X1 Y1 X2 Y2 [fill]
var
  Canvas: TCanvasExt;
begin
  Self := TCompelCanvasExt(Script.Tag);
